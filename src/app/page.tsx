  ...
  return (
    <div className="flex flex-col min-h-screen font-body text-slate-900 overflow-x-hidden bg-background">
      ...
      <main className="flex-1">
        ...
        <section className="relative w-full h-[100svh] overflow-hidden flex items-end justify-center pb-8 md:pb-16">
          ...
          <div className="container relative z-20 px-4 md:px-6 flex flex-col items-center">
            <div className="animate-dropDownLeft bg-white/90 backdrop-blur-lg p-5 md:p-8 text-center rounded-[1.5rem] md:rounded-[2rem] border border-primary/5 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.1)] max-w-4xl w-full">
              ...
              <h1 className="text-lg md:text-3xl lg:text-4xl font-headline font-bold tracking-tighter text-primary uppercase animate-fadeInText-title leading-[1.1] md:leading-[1.2]">
                MUGHER <br className="hidden md:block" /> <span className="text-accent">FULL GOSPEL</span> CHURCH
              </h1>
              <p className="mt-3 md:mt-4 text-slate-600 text-[9px] md:text-sm lg:text-base leading-relaxed max-w-[600px] mx-auto animate-fadeInText-subtitle px-2 font-medium opacity-90">
              ...
              <div className="flex flex-col sm:flex-row gap-2 md:gap-3 pt-4 md:pt-6 justify-center animate-fadeInText-subtitle">
                <Button asChild size="lg" className="w-full sm:w-auto px-6 bg-primary font-bold uppercase tracking-widest shadow-lg hover:scale-105 transition-all h-10 md:h-12 text-[8px] md:text-[10px] text-white">
                  <Link href="/donate">Secure Donation</Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="w-full sm:w-auto px-6 border-primary text-primary hover:bg-primary/5 font-bold uppercase tracking-widest hover:scale-105 transition-all h-10 md:h-12 text-[8px] md:text-[10px]">
        ...
        <section id="sanctuary" className="w-full py-12 md:py-24 bg-white border-b overflow-hidden">
          <div className="container px-4 md:px-6 mx-auto">
            <div className="grid gap-8 lg:grid-cols-2 items-center">
              <div className="relative aspect-video rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-2xl border-4 border-muted/20 w-full transform md:-rotate-1 lg:-rotate-2">
                {sanctuaryImage && (
                  <Image
                ...
                <div className="absolute inset-0 bg-black/5 flex items-center justify-center p-4 md:p-12 text-center backdrop-blur-[1px]">
                  <div className="animate-dropDown">
                    <p className="text-white text-xl md:text-5xl font-black uppercase tracking-[0.4em] md:tracking-[0.6em] drop-shadow-[0_4px_15px_rgba(0,0,0,0.6)] animate-pulse-slow">AMEEN</p>
                  </div>
                </div>
              </div>
        ...
        <section id="events" className="w-full py-12 md:py-24 bg-muted/30">
          <div className="container px-4 md:px-6 mx-auto">
            ...
            <div className="grid gap-4 md:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {loading ? (
                <div className="col-span-full flex justify-center py-20 md:py-24">
                  <Loader2 className="h-8 w-8 md:h-10 md:w-10 animate-spin text-primary" />
                </div>
              ) : events && events.length > 0 ? (
                events.map((event) => (
                  <Card key={event.id} className="border-none shadow-xl hover:shadow-2xl transition-all duration-500 flex flex-col group bg-white rounded-2xl md:rounded-3xl overflow-hidden hover:-translate-y-1 md:hover:-translate-y-2">
                    ...
                    <CardContent className="flex-1 px-5 md:px-6">
                      ...
                      <p className="text-[9px] md:text-[12px] line-clamp-3 text-slate-600 leading-relaxed font-medium">
                        {event.description}
                      </p>
                    </CardContent>
                    <CardFooter className="pt-0 pb-5 md:pb-6 px-5 md:px-6">
                      {mounted && (
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button 
                              variant="ghost" 
                              className="w-full group/btn gap-2 text-primary hover:bg-primary/5 p-0 justify-start font-bold uppercase text-[7px] md:text-[9px] tracking-[0.2em] border-t pt-4 mt-1"
                            >
                          ...
                          <DialogContent className="max-w-[95vw] sm:max-w-3xl border-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] rounded-[1.5rem] md:rounded-[2rem] p-0 overflow-hidden">
                            ...
                            <div className="p-6 md:p-12 space-y-6 md:space-y-8 max-h-[70vh] overflow-y-auto">
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                                <div className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-muted/50 border border-muted shadow-inner">
                                  <p className="font-bold text-[7px] md:text-[8px] uppercase tracking-widest text-muted-foreground mb-1">Date</p>
                                  <p className="font-bold text-primary flex items-center gap-2 text-xs md:text-sm"><CalendarIcon className="h-3 w-3" /> {event.date}</p>
                                </div>
                                <div className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-muted/50 border border-muted shadow-inner">
                                  <p className="font-bold text-[7px] md:text-[8px] uppercase tracking-widest text-muted-foreground mb-1">Time</p>
                                  <p className="font-bold text-primary flex items-center gap-2 text-xs md:text-sm"><Clock className="h-3 w-3" /> {event.time}</p>
                                </div>
                                <div className="p-3 md:p-4 rounded-xl md:rounded-2xl bg-muted/50 border border-muted shadow-inner">
                                  <p className="font-bold text-[7px] md:text-[8px] uppercase tracking-widest text-muted-foreground mb-1">Location</p>
                                  <p className="font-bold text-primary flex items-center gap-2 truncate text-xs md:text-sm"><MapPin className="h-3 w-3" /> {event.location}</p>
                                </div>
                              </div>
                              <div className="space-y-3 md:space-y-4">
        ...
        <section id="features" className="w-full py-12 md:py-24 bg-white">
          <div className="container px-4 md:px-6 mx-auto">
            <div className="flex flex-col items-center justify-center space-y-3 md:space-y-4 text-center mb-10 md:mb-16">
              <h2 className="text-2xl md:text-5xl lg:text-6xl font-headline font-bold tracking-tighter text-primary uppercase">Management Integrity</h2>
              <p className="max-w-[700px] text-muted-foreground text-[9px] md:text-lg font-medium px-4 opacity-70">
                Powered by SANCTUARYLINK. Modern tools for a timeless mission.
              </p>
            </div>
            <div className="mx-auto grid max-w-6xl gap-6 md:gap-8 lg:grid-cols-3 px-4">
              ...
              <div className="group flex flex-col items-center space-y-4 md:space-y-5 p-6 md:p-7 rounded-[1.5rem] md:rounded-[2rem] border border-muted bg-white shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1 md:hover:-translate-y-2">
                <div className="bg-primary/5 p-3 md:p-4 rounded-2xl group-hover:bg-primary transition-colors">
                  <PieChart className="h-6 w-6 md:h-8 md:w-8 text-primary group-hover:text-white" />
                </div>
        ...
        {/* CTA Section */}
        <section className="w-full py-12 md:py-24 bg-primary text-white overflow-hidden relative">
          <div className="container px-4 md:px-6 mx-auto flex flex-col items-center space-y-6 md:space-y-8 text-center relative z-10">
            <div className="bg-white/10 p-3 md:p-4 rounded-full backdrop-blur-lg border border-white/10 animate-pulse-slow shadow-2xl">
              <ShieldCheck className="h-6 w-6 md:h-12 md:w-12 text-accent" />
            </div>
            <h2 className="text-xl md:text-5xl font-headline font-bold tracking-tighter uppercase leading-[0.9] md:leading-[1]">Trust Through <br /> <span className="text-accent">Transparency</span></h2>
            <p className="max-w-[650px] text-primary-foreground/80 text-[9px] md:text-base leading-relaxed px-4 font-medium italic opacity-90">
              "Every contribution at MUGHER FULL GOSPEL CHURCH is verified and handled with the highest integrity."
            </p>
            <div className="pt-2 w-full sm:w-auto">
              <Button asChild size="lg" className="w-full sm:w-auto bg-accent text-accent-foreground hover:bg-white px-8 md:px-10 h-10 md:h-12 font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] shadow-[0_15px_40px_rgba(0,0,0,0.3)] hover:scale-105 transition-all rounded-full">
                <Link href="/donate">Support Our Mission</Link>
              </Button>
            </div>
          </div>
          <div className="absolute top-0 left-0 w-48 md:w-[600px] h-48 md:h-[600px] bg-accent/10 rounded-full blur-[100px] md:blur-[180px] -translate-x-1/3 -translate-y-1/3 opacity-30" />
          <div className="absolute bottom-0 right-0 w-64 md:w-[800px] h-64 md:h-[800px] bg-accent/10 rounded-full blur-[120px] md:blur-[220px] translate-x-1/4 translate-y-1/4 opacity-30" />
        </section>
      </main>
      <footer className="flex flex-col gap-6 py-8 md:py-14 w-full shrink-0 items-center px-4 md:px-6 border-t bg-white sm:flex-row">
        <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-5">
          ...
          <div className="text-center sm:text-left">
            <p className="text-[7px] md:text-[9px] font-bold uppercase tracking-widest text-primary mb-0.5">MUGHER FULL GOSPEL CHURCH</p>
            <p className="text-[6px] md:text-[8px] font-bold uppercase tracking-widest text-muted-foreground opacity-50">© 2024 SANCTUARYLINK • FAITH IN ACTION</p>
          </div>
        </div>
        <nav className="flex gap-4 md:gap-6 sm:ml-auto">
          <Link className="text-[6px] md:text-[8px] font-black uppercase tracking-[0.2em] hover:text-primary transition-colors" href="#">
            Privacy
          </Link>
          <Link className="text-[6px] md:text-[8px] font-black uppercase tracking-[0.2em] hover:text-primary transition-colors" href="#">
            Terms
